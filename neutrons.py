...
def Style():
    ...
    ROOT.gROOT.UseCurrentStyle()      # this makes everything black and white, removing the default red border on images    
    ROOT.gROOT.ForceStyle()           # forces the style chosen above to be used, not the style the rootfile was made with
...
def main():
    ...
    h_energy_residual  = TH1F ("neutron_energy_residual", "neutron energy residual (fraction of neutron energy)", 100, -0.4, 0.4)

    h_phi             = TH1F ("proton_phi", "proton phi",     100, -3.2, 3.2)
    h_theta           = TH1F ("proton_theta", "proton theta", 100, 0.0, 3.2)
    h_angle_xaxis     = TH1F ("proton_angle_x", "proton angle wrt x-axis", 100, -3.2, 3.2)
    h_cos_angle_xaxis = TH1F ("proton_cos_angle_x", "proton cos(angle wrt x-axis)", 100, -3.2, 3.2)
    h_energy          = TH1F ("proton_energy", "proton energy (MeV)", 100, 0, 6)                        
    h_energy_xaxis    = TH1F ("proton_energy_x", "proton angle wrt x-axis, wheighted by energy", 100, -3.2, 3.2)
    h_theta_vs_phi    = TH2D ("proton_theta_vs_phi",";#Theta (degrees);#Phi (degrees);recoils per bin", 25, 0 , 180., 25, -180., 180. )
    ...
    for kinetic_energy in neutron_energies:
        ...
        for n_neutrons in n_neutrons_detected:
            ...
            for iteration in range(iterations):
                ...
                true_protons = proton_scattering (random_engine, n_neutrons, incoming_neutron, gas) 
                ...
                if debug is True:
                    ...
                    for proton in reco_protons:
                        ...
                        h_theta_vs_phi.Fill(proton.Theta()*todeg,proton.Phi()*todeg)
                        h_theta_vs_phi_weighted.Fill(proton.Theta()*todeg, proton.Phi()*todeg, energy)                 
            
                    for histogram in [h_phi, h_theta, h_angle_xaxis, h_energy_xaxis, h_energy, h_cos_angle_xaxis]:
                        histogram.Draw()
                        c1.Print(histogram.GetName()+".png")
                    ...
                    for histogram in [h_theta_vs_phi, h_theta_vs_phi_weighted]:
                        ...
                        c1.Print(histogram.GetName()+"_lego.png")

                    for histogram in [h_theta_vs_phi, h_theta_vs_phi_weighted]:
                        histogram.Draw("surf3polz")
                        c1.Print(histogram.GetName()+"_surf3polz.png")
            c1.cd()
            for histogram in [h_angle_residual, h_energy_residual]:
                histogram.Draw()
                histogram.Fit("gaus")
                c1.Print(histogram.GetName()+ "_" + str(kinetic_energy) + "MeV_" + str(n_neutrons) + "neutrons.png")
...
def find_beam_direction(protons, neutron_energy, debug=False):
    ...
    for proton in protons:
        ...
        dr           += [todeg*proton.Angle(TVector3(1,0,0))] # angle w.r.t. beam axis --> FIXME: make beam axis a variable here and in other parts of code!
        dr_sigma     += [sqrt(sigma_phi_deg**2+sigma_theta_deg**2)] # FIXME: just use a two degree sigma for now, study dependence later to see how much it affects final resolution
        if debug is True:
            print("proton kin, phi, theta, dr, sigma_e =", kin_energy, proton.Phi(), proton.Theta(), proton.Angle(TVector3(1,0,0)), sigma_energy(kin_energy))
    ...
    gr_phi.SetMarkerColor(4); gr_phi.SetMarkerStyle(21);
    gr_theta.SetMarkerColor(4); gr_theta.SetMarkerStyle(21);
    gr_dr.SetMarkerColor(4); gr_dr.SetMarkerStyle(21);


    if debug is True:
        c2 = TCanvas("c2","A Simple Graph Example",200,10,700,500)
        c2.cd()
        gr_phi.Draw("AP");   c2.Print("graph_phi.png")
        gr_theta.Draw("AP"); c2.Print("graph_theta.png")
...
#   3  c3           5.77298e-01   9.09558e-02   3.22840e-06  -8.78497e-04
    ...
    myfit = TF1("myfit","[0]*(1.0 + 0.0707283*0.017453*(x-[1]) - 1.35491*(0.017453*(x-[1]))**2 + 0.577298*(0.017453*(x-[1]))**3)", 0, pi/2);
...
def proton_detection (random_engine, true_protons):
    ...
    for proton in true_protons:
        ...
        direction.SetMag(sqrt(new_energy**2-mass**2))  # keeping theta and phi         
